...
public class StudentReader {
	...
	private static Student processLine(String line) {
		...
		String last = s1.next();
		String unity = s1.next();
		int id = Integer.valueOf(s1.next());
		double gpa = Double.valueOf(s1.next());
		int hours = Integer.valueOf(s1.next());
		
		s1.close();
		return new Student(first, last, id, hours, gpa, unity);
	}
}
