...
public class SearchTableMap<K extends Comparable<K>, V> extends AbstractOrderedMap<K, V> {
	...
	/** The ArrayBasedList object used in implementing a SearchTableMap. */
    ...
    private int binarySearchHelper(int min, int max, K key) {
        ...
        K currentKey = list.get(mid).getKey();
        if(currentKey.equals(key)) {
        	return mid;
        } else if(compare(currentKey, key) > 0) {
    ...
    public V put(K key, V value) {
        int index = lookUp(key);
        if(index >= 0) { // Key already exists, update with new value and return old value
    ...
    public V remove(K key) {
        int index = lookUp(key);
        if(index < 0) { // Key does not exist, return null
        	return null;
        }
