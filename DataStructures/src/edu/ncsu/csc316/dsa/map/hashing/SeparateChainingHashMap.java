...
public class SeparateChainingHashMap<K extends Comparable<K>, V> extends AbstractHashMap<K, V> {
	...
	/** Hash table object which is an array of secondary maps(buckets). */
    ...
    public V bucketPut(int hash, K key, V value) {
        ...
        // If there is no map in the bucket, a new map needs to be created
        if(bucket == null) {
        	table[hash] = new RedBlackTreeMap<K, V>();
        	table[hash].put(key, value);
        	return null;
        } else {
        	// If there is a map, put the entry into the existing map.
        	V oldVal = bucket.get(key);
        	bucket.put(key, value);
        	
        	// Return null or replaced value
        	return oldVal;
        }
    }
    ...
    public V bucketRemove(int hash, K key) {
        ...
        // If there is no bucket, the key does not exist
        if(bucket == null) {
        	return null;
        }
        
        // Remove the value and store it
        V removed = bucket.remove(key);
        
        // If the removed value is not null, there was an element removed
        // and size needs to be decremented
        if(removed != null) {
        	size--;
        }
