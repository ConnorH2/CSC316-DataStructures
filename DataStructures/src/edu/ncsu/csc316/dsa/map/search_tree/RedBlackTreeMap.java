...
public class RedBlackTreeMap<K extends Comparable<K>, V> extends BinarySearchTreeMap<K, V> {
    ...
    private void resolveRed(Position<Entry<K, V>> p) {
        ...
        BSTNode<Entry<K, V>> parent = (BSTNode<Entry<K, V>>) parent(node);
        if(isRed(parent)) {
        	BSTNode<Entry<K, V>> uncle = (BSTNode<Entry<K, V>>) sibling(parent);
        	// Case 1: the uncle (sibling of the parent) is black.
        	if(isBlack(uncle)) {
        		BSTNode<Entry<K, V>> middle = (BSTNode<Entry<K, V>>) restructure(node);
        		makeBlack(middle);
        		makeRed(left(middle));
        		...
        		makeBlack(parent);
        		makeBlack(uncle);
        		BSTNode<Entry<K, V>> grandparent = (BSTNode<Entry<K, V>>) parent(parent);
    ...
    private void remedyDoubleBlack(Position<Entry<K, V>> p) {
    	BSTNode<Entry<K, V>> node = (BSTNode<Entry<K, V>>) p;
    	BSTNode<Entry<K, V>> parent = (BSTNode<Entry<K, V>>) parent(node);
    	BSTNode<Entry<K, V>> sibling = (BSTNode<Entry<K, V>>) sibling(node);
    	
    	if(isBlack(sibling)) {
    		// Case 1: trinode restructuring
    		if(isRed(left(sibling)) || isRed(right(sibling))) {
    			BSTNode<Entry<K, V>> temp = (BSTNode<Entry<K, V>>)(isRed(left(sibling)) ? left(sibling) : right(sibling));
    			BSTNode<Entry<K, V>> middle = (BSTNode<Entry<K, V>>) restructure(temp);
    			if(isRed(parent)) {
    				makeRed(middle);
    			} else {
    				makeBlack(middle);
