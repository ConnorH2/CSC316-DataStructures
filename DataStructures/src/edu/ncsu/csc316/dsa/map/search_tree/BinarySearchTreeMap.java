...
public class BinarySearchTreeMap<K extends Comparable<K>, V> extends AbstractOrderedMap<K, V> {
	...
	/** Private inner class for use in implementing BinarySearchTreeMap. */
     ...
     * @param compare The comparator to use when comparing keys within the tree.
     */
    ...
    protected static class BalanceableBinaryTree<K, V> extends LinkedBinaryTree<Entry<K, V>> {
        ...
        public void rotate(Position<Entry<K, V>> p) {
            ...
            BinaryTreeNode<Entry<K, V>> nodeParent = (BinaryTreeNode<Entry<K, V>>) parent(node);
            BinaryTreeNode<Entry<K, V>> nodeGrandparent = (BinaryTreeNode<Entry<K, V>>) parent(nodeParent);
            ...
            if(nodeGrandparent == null) {
            	...
            	if(nodeParent.equals(left(nodeGrandparent))) {
            		relink(nodeGrandparent, node, true);
            	} else {
            		relink(nodeGrandparent, node, false);
            	}
            }
            // Regardless of whether a grandparent exists,
            // relink the parent and node and transfer node's subtree
            if(node.equals(left(nodeParent))) {
            	relink(nodeParent, (BinaryTreeNode<Entry<K, V>>) right(node), true);
            	relink(node, nodeParent, false);
            } else {
            	relink(nodeParent, (BinaryTreeNode<Entry<K, V>>) left(node), false);
            	relink(node, nodeParent, true);
            }
        }
        ...
        public Position<Entry<K, V>> restructure(Position<Entry<K, V>> x) {
            ...
            // Node and parent same side, one rotation
            if( node.equals(left(nodeParent)) && nodeParent.equals(left(nodeGrandparent)) || 
            		node.equals(right(nodeParent)) && nodeParent.equals(right(nodeGrandparent)) ) {
            	rotate(nodeParent);
            	return nodeParent;
            } else { // node and parent different side, two rotations
            	rotate(node);
            	rotate(node);
            	return node;
            }
        ...
        protected static class BSTNode<E> extends BinaryTreeNode<E> {
        	...
        	/** An additional property of the BSTNode which is used in some search trees. */
