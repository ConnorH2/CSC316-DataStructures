...
public class LinkedBinaryTree<E> extends AbstractBinaryTree<E> {
	...
	/** The root node of the LinkedBinaryTree. */
    private BinaryTreeNode<E> root;
    /** The total size of the LinkedBinaryTree. */
    private int size;
    ...
    public static class BinaryTreeNode<E> extends AbstractTreeNode<E> {
    	/** The node above the current BinaryTreeNode. */
        private BinaryTreeNode<E> parent;
        /** The child node to the left of the current BinaryTreeNode. */
        private BinaryTreeNode<E> left;
        /** The child node to the right of the current BinaryTreeNode. */
    ...
    public E remove(Position<E> p) {
        ...
        // Test if node has already been removed
        // removed if the node is not the root and its parent is null
    	if(!root.equals(node) && parent(node) == null) {
    		return null;
    	}
        ...
        if(root.equals(node)) {
        	// Removing root node, replace root with child or null if no child
        	...
        	root = child;
        	//  If root is not null, set its parent to be null
        	if(root != null) {
        		root.setParent(null);
        	}
        } else {
        	// Not removing root node, there is a node above the one being removed
        	BinaryTreeNode<E> nodeParent = node.getParent();
        	// Set parent to point to child or null if no child
        	if(node.getLeft() != null) {
        		child = node.getLeft();
        	} else {
        		child = node.getRight();
        	...
        	// Set node's old space to be occupied by child
        	...
        	// If child is not null, set it to point to parent
        	if(child != null) {
        		child.setParent(nodeParent);
        	}
        	
        	// Set node's parent to null so we can easily detect if it has been removed
        	node.setParent(null);
        }
        size--;
        // Either way, return the removed value
