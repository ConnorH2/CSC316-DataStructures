...
public class MinimumSpanningTreeUtil {
    ...
    public static <V, E extends Weighted> PositionalList<Edge<E>> kruskal(Graph<V, E> g) {
        PositionalList<Edge<E>> mstEdges = new PositionalLinkedList<Edge<E>>();
        PriorityQueue<Integer, Edge<E>> sortedEdges = new HeapPriorityQueue<Integer, Edge<E>>();
        DisjointSetForest<Vertex<V>> sets = new UpTreeDisjointSetForest<Vertex<V>>();
        
        // Insert the edges into the priority queue
        for(Edge<E> e : g.edges())
        {
        	sortedEdges.insert(e.getElement().getWeight(), e);
        }
        
        // Create each vertex as its own component/disjoint set
        ...
        while(components > 1)
        {
        	// Get the edge w/ lowest weight
        	Entry<Integer, Edge<E>> entry = sortedEdges.deleteMin();
