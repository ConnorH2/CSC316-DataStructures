...
public class AdjacencyListGraph<V, E> extends EdgeListGraph<V, E> {
    ...
    public Iterable<Edge<E>> incomingEdges(Vertex<V> vertex) {
    	// Cast the vertex to an ALVertex
    ...
    public Edge<E> insertEdge(Vertex<V> vertex1, Vertex<V> vertex2, E edgeData) {
    	// Create the new edge
    	ALEdge e = (ALEdge) super.insertEdge(vertex1, vertex2, edgeData);
    	
    	ALVertex origin = validate(vertex1);
        ALVertex destination = validate(vertex2);
        
        // Link the edge to its outgoing and incoming positions
        e.setOutgoingListPosition(origin.outgoing.addLast(e));
        e.setIncomingListPosition(destination.incoming.addLast(e));
    ...
    public void removeEdge(Edge<E> edge) {
    	// Call EdgeListGraph.removeEdge first
    	super.removeEdge(edge);
    	// Get the edge's origin and destination
        ALEdge e = validate(edge);
        ALVertex origin = validate(e.getEndpoints()[0]);
        ALVertex destination = validate(e.getEndpoints()[1]);
        
        // Unlink the edge from the origin and destination
