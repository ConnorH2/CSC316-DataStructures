...
public class HeapAdaptablePriorityQueue<K extends Comparable<K>, V> extends HeapPriorityQueue<K, V>
    ...
    public void remove(Entry<K, V> entry) {
    	AdaptablePQEntry<K, V> aPQEntry = validate(entry);
    	
    	int idx = aPQEntry.index;
    	
    	// Swap the last entry in the list with the one being removed
    	swap(idx, list.size() - 1);
    	
    	// Then remove the new last element in the list
    	list.removeLast();
    	
    	// Then up or down-heap with bubble
    	bubble(idx);
    }
    ...
    public void replaceKey(Entry<K, V> entry, K key) {
    	...
    	// Replace the key, then up or down heap with bubble
    	aPQEntry.setKey(key);
    	bubble(aPQEntry.index);
    }
